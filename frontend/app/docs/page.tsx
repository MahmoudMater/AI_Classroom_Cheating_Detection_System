...
export default function DocsPage() {
  return (
    <DocumentationLayout>
      {/* Hero */}
      <div className="relative overflow-hidden rounded-2xl md:rounded-[32px] border border-blue-900/30 bg-gradient-to-br from-[#0f2040] via-[#0d1b3e] to-[#0a1228] p-8 md:p-16 mb-12">
        <div className="absolute -top-20 -right-20 size-64 md:size-96 rounded-full bg-blue-500/10 blur-[100px] pointer-events-none" />
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-[10px] md:text-xs font-bold text-blue-400 uppercase tracking-widest mb-6">
            <span className="size-1.5 rounded-full bg-blue-500 animate-pulse" />
            Computer Vision · Binary Classification
          </div>
          <h1 className="text-3xl md:text-6xl font-extrabold text-white leading-[1.1] mb-6 font-syne tracking-tighter">
            Exam Cheating <br />
            <span className="text-blue-500">Detection System</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-3">
            {["Custom CNN", "ResNet18", "EfficientNet-B0", "ViT-B/16"].map((m) => (
              <Tag key={m} color="#3b82f6">{m}</Tag>
            ))}
            <Tag color="#22c55e">97.80% Accuracy</Tag>
            <Tag color="#a78bfa">ROC-AUC 0.9999</Tag>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-blue-600/5 border border-blue-600/10 rounded-2xl p-6 md:p-8">
          <div className="flex items-center gap-3 mb-3">
            <div className="size-6 rounded-full bg-blue-600/20 flex items-center justify-center text-blue-500 font-bold text-xs">!</div>
            <h4 className="text-sm font-bold text-slate-200 uppercase tracking-widest">Why Custom CNN?</h4>
          </div>
      ...
      <div className="space-y-4">
        {[
          ...
          <div key={i} className="flex flex-col md:flex-row gap-6 p-6 rounded-2xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.04] transition-colors">
            <div className="size-12 rounded-xl bg-blue-600/10 border border-blue-600/30 flex items-center justify-center text-blue-500 font-bold text-lg shrink-0">
              {i + 1}
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                <span className="text-xl">{step.icon}</span>
                <h4 className="text-base font-bold text-slate-100 font-syne uppercase tracking-tight">NB {step.nb} — {step.title}</h4>
                {step.next && <span className="px-2 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-500 text-[10px] font-bold uppercase">Next Step</span>}
              </div>
              ...
              <div className="flex flex-col md:flex-row gap-4 md:gap-8 font-mono text-[11px] md:text-xs">
                <div className="flex gap-2 items-center">
                  ...
                  <code className="text-blue-400">{step.input}</code>
                </div>
                <div className="flex gap-2 items-center">
                  <span className="text-slate-600 font-bold uppercase">Output:</span>
                  <code className="text-green-400">{step.output}</code>
                </div>
              </div>
            </div>
          </div>
...
if not is_blurry(frame, BLUR_THRESHOLD):
    ...
    saved += 1`} />
      ...
      <div className="space-y-6">
            ...
            <thead>
              <tr className="border-b border-white/10 bg-white/5">
                {["Rank", "Model", "Accuracy", "F1-Score", "ROC-AUC", "Params", "Status"].map((h) => (
                  <th key={h} className="px-6 py-4 font-bold text-blue-500 uppercase tracking-widest">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {MODEL_COMPARISON.map((row, i) => (
                <tr key={i} className={i === 0 ? "bg-blue-500/5" : "hover:bg-white/[0.02] transition-colors"}>
                  <td className="px-6 py-4 font-bold" style={{ color: row.color }}>{row.rank}</td>
                  <td className="px-6 py-4 text-slate-100 font-bold">{row.name}</td>
                  <td className="px-6 py-4 text-slate-400">{row.acc}</td>
                  <td className="px-6 py-4 text-slate-400">{row.f1}</td>
                  <td className="px-6 py-4 font-bold" style={{ color: row.aucColor }}>{row.auc}</td>
                  <td className="px-6 py-4 text-slate-500">{row.params}</td>
        ...
        <div className="bg-green-500/5 border border-green-500/10 rounded-2xl p-6 md:p-8">
          <div className="flex items-center gap-3 mb-3">
            <div className="size-6 rounded-full bg-green-500/20 flex items-center justify-center text-green-500 font-bold text-xs">⭐</div>
            <h4 className="text-sm font-bold text-slate-200 uppercase tracking-widest">Final Verdict</h4>
          </div>
          <p className="text-sm text-slate-500 leading-relaxed italic">
            <strong className="text-green-400">Custom CNN</strong> is the deployment model.
      ...
      <footer className="mt-24 pt-12 border-t border-white/10">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="text-center md:text-left">
            <h5 className="text-slate-100 font-bold mb-2">Integrity Engine ML Docs</h5>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Tag color="#22c55e">97.80% Accuracy</Tag>
            <Tag color="#3b82f6">Custom CNN</Tag>
            <Tag color="#a78bfa">AUC 0.9999</Tag>
          </div>
        </div>
        <div className="text-center mt-12 text-[10px] text-slate-700 uppercase tracking-[0.4em] font-mono">
