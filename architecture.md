...
graph TD
    subgraph ENTRY["Entry Points"]
        E1[REST Client<br/>Next.js frontend]
        E2[WebSocket Client<br/>LiveFeed canvas]
    end

    subgraph MAIN["main.py — FastAPI App"]
        MW[CORS Middleware]
        LF[Lifespan: startup / shutdown]
        EH[Global 500 handler]
    end

    subgraph ROUTERS["Routers /api/v1/"]
        R1[sessions.py<br/>POST /sessions<br/>GET /sessions<br/>POST /{id}/start<br/>POST /{id}/stop<br/>DELETE /{id}]
        R2[events.py<br/>GET /{id}/events<br/>GET /{id}/events/summary<br/>GET /{id}/events/export]
        R3[stream.py<br/>WS /ws/{session_id}]
        R4[health.py<br/>GET /health]
        R5[models_router.py<br/>GET /api/v1/models]
        R6[analyze.py<br/>POST /api/v1/analyze-image]
    end

    subgraph SERVICES["Services"]
        S1[session_manager.py<br/>Dict: session_id → ProctoringService]
        S2[proctoring_service.py<br/>Threading wrapper around proctoring.py]
        S3[frame_broadcaster.py<br/>asyncio bridge: thread → WebSocket]
        S4[event_store.py<br/>Async reads + Sync writes from CV thread]
    end

    subgraph DB["Database"]
        DB1[(PostgreSQL 16<br/>exam_sessions<br/>proctoring_events)]
        DB2[SQLAlchemy 2.x async<br/>asyncpg driver]
        DB3[SQLAlchemy sync<br/>psycopg2 driver<br/>for CV thread writes]
    end
...
sequenceDiagram
    ...
    participant WS as WebSocket<br/>(/ws/{id})
    ...
    loop Every Frame
        ...
        WS->>FE: JSON { type:"frame", frame_b64, persons[], summary }
...
sequenceDiagram
    ...
    FE->>WS: Connect ws://localhost:8000/ws/{session_id}
    WS-->>FE: { type: "status", status: "started", message: "..." }

    loop While session running (up to 30fps)
        WS-->>FE: {<br/>  type: "frame",<br/>  session_id, frame_index, fps,<br/>  timestamp (ISO),<br/>  frame_b64 (JPEG base64),<br/>  persons: [{id, verdict, cheat_prob,<br/>    direction, obj_nearby, obj_name,<br/>    reasons[], bbox[x1,y1,x2,y2]}],<br/>  summary: {person_count, cheating_count}<br/>}
    end

    FE->>WS: { type: "ping" }
    WS-->>FE: { type: "pong" }

    WS-->>FE: { type: "status", status: "stopped", message: "..." }
    FE->>WS: Disconnect
```
