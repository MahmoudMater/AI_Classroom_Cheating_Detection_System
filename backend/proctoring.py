...
GREEN  = (0, 210, 0);  RED    = (0, 0, 220)
ORANGE = (0, 140, 255); WHITE = (255, 255, 255)
BLACK  = (0, 0, 0);    YELLOW = (0, 210, 255)
...
model_path = os.path.join(DRIVE_MODELS, BEST_MODEL_FILE)
...
def get_direction_from_keypoints(keypoints):
    ...
    if nose[2] > 0.3 and left_sh[2] > 0.3 and right_sh[2] > 0.3:
        ...
        if   norm_x < HEAD_L:           direction = "LEFT"
        elif norm_x > HEAD_R:           direction = "RIGHT"
        elif nose[1] > mid_y * HEAD_DOWN: direction = "DOWN"
    if left_ear[2] > 0.3 and right_ear[2] > 0.3:
        ear_ratio = (nose[0] - right_ear[0]) / (left_ear[0] - right_ear[0] + 1e-6)
        if   ear_ratio < GAZE_L: direction = "GAZE LEFT"
        elif ear_ratio > GAZE_R: direction = "GAZE RIGHT"
    elif left_ear[2] > 0.3 and right_ear[2] < 0.2:  direction = "GAZE RIGHT"
    elif right_ear[2] > 0.3 and left_ear[2] < 0.2:  direction = "GAZE LEFT"
    return direction

# ── Event Log ────────────────────────────────────────────────────────
with open(LOG_PATH, "w", newline="") as f:
    csv.writer(f).writerow(["timestamp","person_id","verdict","cheat_prob","direction","obj_nearby","obj_name","reasons"])

def log_event(pid, verdict, cheat_p, direction, obj_near, obj_name, reasons):
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(LOG_PATH, "a", newline="") as f:
        csv.writer(f).writerow([ts, pid, "CHEATING" if verdict else "OK",
                                 f"{cheat_p:.4f}", direction, obj_near, obj_name,
                                 " | ".join(reasons)])

# ── Person State ─────────────────────────────────────────────────────
class PersonState:
    __slots__ = ["dir_timer","direction","clf_pred","cheat_prob","cheating","obj_near","obj_name","reasons","frame_count"]
    def __init__(self):
        self.dir_timer=0; self.direction="FRONT"; self.clf_pred=1
        self.cheat_prob=0.0; self.cheating=False; self.obj_near=False
        self.obj_name=""; self.reasons=[]; self.frame_count=0
...
out_vid = cv2.VideoWriter(
    ...
    cv2.VideoWriter_fourcc(*"XVID"), fps_cap, (W, H)
)
...
while True:
    ...
    for (pid, px1, py1, px2, py2) in person_boxes:
        ...
        st.obj_near = False; st.obj_name = ""
        for (olbl, ox1, oy1, ox2, oy2) in object_boxes:
            if object_proximity((px1,py1,px2,py2), (ox1,oy1,ox2,oy2), DIAG):
                st.obj_near = True; st.obj_name = olbl
                st.reasons.append(f"{olbl} nearby"); break
        ...
        if kp is not None:
            for idx in [0, 3, 4, 5, 6]:
                x, y, conf = kp[idx]
                if conf > 0.3: cv2.circle(frame, (int(x), int(y)), 4, (200,200,200), -1)

        if frame_idx % CLF_EVERY_N == 0:
            pred, cp, _ = classify_roi(roi)
            st.clf_pred = pred; st.cheat_prob = cp

        clf_flag = st.cheat_prob >= CLF_THRESHOLD
        if clf_flag: st.reasons.append(f"clf {st.cheat_prob:.0%}")
        ...
        cv2.putText(frame, f"P{pid} | {label}", (px1+4,py1-6), cv2.FONT_HERSHEY_SIMPLEX, 0.5, WHITE, 1, cv2.LINE_AA)
        iy = py2 + 16
    ...
    out_vid.write(frame)
    cv2.imshow("AI Proctoring", frame)   # ← This opens the live window
